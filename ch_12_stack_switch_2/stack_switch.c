...
int main(void)
{
	...
	__set_CONTROL(__get_CONTROL()|0x2); // Set SPSEL, Do not change other bits
	__set_MSP(tmp);                     // Move MSP to point to Handler stack
	
	puts("After stack switch");
	printf ("-MSP = %x\n", __get_MSP());
	printf ("-PSP = %x\n", __get_PSP());
	printf ("-CONTROL = %x\n", __get_CONTROL());
